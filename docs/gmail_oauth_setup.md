...
## ★ client_secret について（重要・必須）

> **[2026-06-28 訂正]** 前バージョンでは「PKCE 方式のため client_secret 不要」と
> 記載していたが、**これは誤りでした**。実際にトークン交換を走らせたところ Google が
> `400 invalid_request: client_secret is missing` を返した。
> 当該クライアント（recruit-gmail-oauth3・デスクトップアプリ型）は **token 交換に
> client_secret が必須**。PKCE（RFC 7636）は必要条件だが十分条件ではない（両方必要）。

**PKCE + client_secret の両方が必要です。** GCP からクライアント JSON をダウンロードして
`client_secret` の値を取得してください。

client_secret の供給方法（スクリプトが優先順に読み込む）:
1. 環境変数 `GMAIL_OAUTH_CLIENT_SECRET`（値を直接指定）
2. 環境変数 `GMAIL_OAUTH_CLIENT_JSON`（GCPからダウンロードしたJSONファイルのパス）
   → JSON 内の `installed.client_secret` と `installed.client_id` が自動で読み込まれる（推奨）
3. コマンドライン引数 `python scripts/get-gmail-refresh-token.py <client_id> <client_secret>`

生の client_secret を `.bat` や設定ファイルに直書きしないこと。JSON ファイルまたは環境変数で渡す。
...
> **注**: `src/gmail_oauth.py` の refresh（`grant_type=refresh_token`）では
> client_secret は "Optional" として動作する可能性がある（既存の refresh token を
> 使い続けるフェーズ）。ただし初回の token 交換（authorization_code フロー）では必須。
...
```bash
# JSON ファイルを使う場合（推奨・client_id も自動で読み込まれる）:
GMAIL_OAUTH_CLIENT_JSON=~/recruit-gmail-oauth3.json \
...
> **サーバーの堅牢性について**: スクリプトはループ型のローカルサーバーを起動するため、
> ブラウザが送る favicon リクエスト等の中間リクエストでサーバーが停止することはない。
> `?code=` を含むコールバックが来るまで待機を継続する。
...
取得した3値を recruit サービス（Railway `desirable-smile / recruit`）の Variables に設定する。
