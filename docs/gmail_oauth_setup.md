...
| 条件 | 使う認証方式 |
|------|-------------|
| `GMAIL_OAUTH_CLIENT_ID` / `GMAIL_OAUTH_CLIENT_SECRET` / `GMAIL_OAUTH_REFRESH_TOKEN` が全て設定済み | **OAuth2 (IMAP XOAUTH2)** |
| 上記が未設定 | アプリパスワード IMAP（`GMAIL_IMAP_PASSWORD`） |

→ OAuth の3変数を入れた瞬間に OAuth へ切り替わる。問題があれば3変数を消せばアプリパスワードに戻る。
...
| `GMAIL_OAUTH_CLIENT_SECRET` | GCP OAuth クライアントシークレット | OAuth時○ |
...
## A. GCP で OAuth クライアントを作る（client_id / client_secret）

> 既存の takagi.bz GCP プロジェクトに OAuth クライアントがあれば流用してもよい
> （種別が「デスクトップアプリ」で、redirect に `http://127.0.0.1:8765/` を許可できるものなら可）。
> 流用できない場合は以下で新規作成する。

1. [Google Cloud Console](https://console.cloud.google.com/) を `recruit@takagi.bz` を管理できる
   アカウント（または takagi.bz 組織管理者）で開く。
2. プロジェクトを選択（社内共通の GCP プロジェクトを推奨）。
3. **API とサービス → ライブラリ** で「Gmail API」を有効化する。
4. **API とサービス → OAuth 同意画面**:
   - User Type は社内利用なら「内部（Internal）」が簡単（takagi.bz 組織アカウントのみ）。
     外部にする場合は **公開ステータスを「本番（In production）」に publish** すること。
     **Testing のままだと refresh token が7日で失効する**（既知の落とし穴）。
   - スコープに `.../auth/gmail.readonly` を追加。
5. **API とサービス → 認証情報 → 認証情報を作成 → OAuth クライアント ID**:
   - アプリケーションの種類: **デスクトップアプリ**
   - 名前: 例 `recruit-gmail-oauth`
...
GMAIL_OAUTH_CLIENT_ID=<クライアントID> \
GMAIL_OAUTH_CLIENT_SECRET=<クライアントシークレット> \
